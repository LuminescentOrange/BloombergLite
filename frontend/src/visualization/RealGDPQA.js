...
const RealGDPQA = () => {
  ...
  let getEarning = async () => {
    let response = await fetch(`/lookup/quarterly_real_gdp/`)
    let data = await response.json()
    setEarning(data)
  }

  let getEarning1 = async () => {
    let response1 = await fetch(`/lookup/annual_real_gdp/`)
